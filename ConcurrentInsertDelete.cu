...
__global__ void Del_Elem(volatile int *heap,int *d_elements,volatile int *curSize,volatile int *lockArr,volatile int *elemSize,volatile int *newSize,int *d_delElem){
...
int main() {
    ...
    for(int lk = 0;lk<100;lk++)
    {
        ...
        *elemSize = getRandom(*curSize,*curSize);
        int delElem[*elemSize],elements[*elemSize],*d_elements;
        ...
        *newSize = *curSize - *elemSize;
        ...
        init<<<1,1>>>(); 
        Del_Elem<<<block,1024>>>(d_a,d_elements,curSize,lockArr,elemSize,newSize,d_delElem);
