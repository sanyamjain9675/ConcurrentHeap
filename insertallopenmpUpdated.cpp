...
void kernel(int heap[],int elements[],int elemSize,int curSize){
    int count = 0;int ind,childInd;

    if(curSize == 0){
        heap[0] = elements[elemSize-1];
        curSize++;
        elemSize--;
    }
...
int main() {
    ...
    for(int lk = 0;lk<100;lk++)
    {
        ...
        int heap[maxSize];
        curSize = getRandom(0,maxSize/10);
        // curSize = 0;
       ...
       //check if satisfies the heap property
        ...
        kernel(heap,elements,elemSize,curSize);
